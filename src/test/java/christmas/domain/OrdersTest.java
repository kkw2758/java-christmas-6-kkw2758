...
class OrdersTest {
    ...
    void ordersSuccessTest() {
        // when & then
        assertDoesNotThrow(() -> Orders.from(Parser.generateOrdersRequest("티본스테이크-1,레드와인-2,해산물파스타-1")));
    }
    @DisplayName("주문 목록에 중복 메뉴를 포함하는 경우 예외 처리한다.")
    @Test
    void duplicateMenuExceptionTest() {
        // when & then
        assertThatThrownBy(() -> Orders.from(Parser.generateOrdersRequest("티본스테이크-1,티본스테이크-1")))
    ...
    void onlyDrinkExceptionTest() {
        // when & then
        assertThatThrownBy(() -> Orders.from(Parser.generateOrdersRequest("제로콜라-1,레드와인-1")))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.");
    }
    ...
    void menuCountExceptionTest() {
        // when & then
        assertThatThrownBy(() -> Orders.from(Parser.generateOrdersRequest("양송이수프-11,레드와인-10")))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.");
    }
    ...
    void calculateTotalPriceBeforeSale() {
        // given
        OrdersRequest ordersRequest = Parser.generateOrdersRequest("티본스테이크-1,레드와인-2,해산물파스타-1");
